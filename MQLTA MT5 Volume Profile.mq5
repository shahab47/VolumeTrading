...
bool OnInitPreChecksPass(){
   if(iBars(Symbol(),VPTimeFrame)<BarsToScan){
      ...
      printf("Insufficient Historical Data, only "+IntegerToString(iBars(Symbol(),VPTimeFrame))+" found in the time frame requested");
      return false;
   }
   return true;
}
...
void VolumeProfileCalculate(){
   ...
   for(int i=0; i<BarsToScan; i++){
      ...
      for(int k=0; k<=CandleSteps; k++){
         ...
         //StepPoints = StepPointsExt*Point(); agar StepPointsExt = 20 gharar bedi yani ihar 20 ta point ye pele beshe
         //steps = yani az balatarin ta paeen tarin gheymat bebin chand point hast hala on ro taghsim bar StepPointsExt mikone 
         // i = masalan migi 5 roz akhar va in miyad 5 ro dar 24 zarb mikone va dar loop ghara mide 
         // k = 
         Comment("Weight = ", Weight," - ",Steps," - ",i," - ",k," - ",MinPrice," - ",MaxPrice," - ",PriceMin," - ",PriceMax," - ",CalcPrice," - ",CandleSteps," - ",h," - ",Point()," l ",StepPoints);
         VolumeProfile[h]+=Weight;
         
      }
      Print("Weight = ",VolumeProfile[1]);
   }
   DrawVolumeProfile();
}
...
void DeleteAllLines(){
   int Window=0;
   for(int i=ObjectsTotal(ChartID(),Window,-1)-1;i>=0;i--){
      if(StringFind(ObjectName(0,i),IndicatorName,0)>=0 && StringFind(ObjectName(0,i),IndicatorName+"-VLINE-",0)>=0){
         ObjectDelete(0,ObjectName(0,i));
      }
   }
}
